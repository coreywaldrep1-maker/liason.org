...
function extractSaved(json) {
  ...
  // If the whole json looks like the saved object (has obvious fields)
  const guessKeys = ["petitioner", "beneficiary", "mailing", "employment", "physicalAddresses"];
  const hasAny = guessKeys.some(k => k in json);
...
async function loadSavedForRequest(request) {
  ...
  if (!userId) {
    ...
    const idFromMe =
      (typeof me?.user?.id === "number" && me.user.id) ||
      (typeof me?.id === "number" && me.id) ||
      (typeof me?.userId === "number" && me.userId) ||
      null;
