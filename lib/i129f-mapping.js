...
// Works with your existing data shape (saved.petitioner, saved.mailing, saved.physicalAddresses, saved.beneficiary.physicalAddress, etc.)
...
export function applyI129fMapping(saved, form) {
  ...
  // Add more Part 3 fields here as needed.
  ...
  setManyChecks(form, ['Pt4_Race_AIAN'],  race.includes('american indian') || race.includes('alaska'));
  setManyChecks(form, ['Pt4_Race_NHPI'],  race.includes('pacific'));
  setManyText(form, ['Pt4_HeightFeet'], bio.heightFeet);
  setManyText(form, ['Pt4_HeightInches'], bio.heightInches);
  setManyText(form, ['Pt4_Weight'], bio.weight);
  const eye = (bio.eyeColor || '').toLowerCase();
  ...
  const hair = (bio.hairColor || '').toLowerCase();
  ...
  setManyText(form, ['Pt5_Email','Pt5_PetitionerEmail'], get(saved, 'petitioner.email'));
  setManyText(form, ['Pt5_Phone_Day'], get(saved, 'petitioner.phone'));
  setManyText(form, ['Pt5_Phone_Mobile'], get(saved, 'petitioner.mobile'));
  ...
  setManyText(form, ['Pt6_Language','Pt6_Interp_Language'], itp.language);
  setManyText(form, ['Pt6_Email','Pt6_Interp_Email'], itp.email);
  setManyText(form, ['Pt6_SignDate','Pt6_Interp_SignDate'], fmtDate(itp.signDate));
  setManyText(form, ['Pt6_Family','Pt6_Interp_Family'], itp.lastName);
  setManyText(form, ['Pt6_Given','Pt6_Interp_Given'], itp.firstName);
  setManyText(form, ['Pt6_Business','Pt6_Interp_Business'], itp.business);
  setManyText(form, ['Pt6_Phone1','Pt6_Interp_Phone1'], itp.phone1);
  setManyText(form, ['Pt6_Phone2','Pt6_Interp_Phone2'], itp.phone2);

  /* ===== Part 7 — Preparer ===== */
  const prep = saved.preparer || {};
  setManyText(form, ['Pt7_Family','Pt7_Prep_Family'], prep.lastName);
  setManyText(form, ['Pt7_Given','Pt7_Prep_Given'],   prep.firstName);
  setManyText(form, ['Pt7_Business','Pt7_Prep_Business'], prep.business);
  setManyText(form, ['Pt7_Phone','Pt7_Prep_Phone'],   prep.phone);
  setManyText(form, ['Pt7_Mobile','Pt7_Prep_Mobile'], prep.mobile);
  setManyText(form, ['Pt7_Email','Pt7_Prep_Email'],   prep.email);
  setManyText(form, ['Pt7_SignDate','Pt7_Prep_SignDate'], fmtDate(prep.signDate));

  /* ===== Part 8 — Additional Information ===== */
  const p8 = saved.part8 || {};
  setManyText(form, ['Pt8_Line3d','Pt8_3d','Additional_3d'], p8.line3d);
  setManyText(form, ['Pt8_Line4d','Pt8_4d','Additional_4d'], p8.line4d);
  setManyText(form, ['Pt8_Line5d','Pt8_5d','Additional_5d'], p8.line5d);
  setManyText(form, ['Pt8_Line6d','Pt8_6d','Additional_6d'], p8.line6d);

  /* ===== Arbitrary overrides (advanced) ===== */
  const other = saved.other || {};
  for (const [fieldName, value] of Object.entries(other)) {
    if (typeof fieldName !== 'string') continue;
    if (!setText1(form, fieldName, value ?? '')) {
