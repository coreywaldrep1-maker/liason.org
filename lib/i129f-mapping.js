...
export function applyI129fMapping(saved = {}, form) {
  ...
  // 13–20. Employment history (uses petitioner.employment[0..1])
  {
    ...
    if (e1) {
      ...
      if (EMP1_UNIT_EXPORT[ut]) {
        safeSelectRadio(form, 'Petitioner_Employment_History_1_Apt_Suite_Floor_Number_Page2_14.b', EMP1_UNIT_EXPORT[ut]);
      }
    ...
    if (e2) {
      ...
      if (EMP2_UNIT_EXPORT[ut]) {
        safeSelectRadio(form, 'Petitioner_Employment_History_2_Apt_Suite_Floor_Number_Page2_18.b', EMP2_UNIT_EXPORT[ut]);
      }
  ...
  // 21–26. Petitioner other info (sex, dob, marital, birth)
  {
    ...
    // Marital Status export values include:
    // Single / Married / Divorced / Widowed (and more)
  ...
  // 27–36. Petitioner parents (uses petitioner.parents[0..1])
  {
    ...
    if (p1) {
      ...
      // Template doesn’t expose Parent city-of-birth in this PDF; fill what exists:
  ...
  // Beneficiary in-US / I-94 / passport section (template spread across page6 + page7)
  {
    ...
    if (yn !== null) {
      // export values in your template:
      // - Beneficiary_Been_in_US_No_page6_37
      // - Beneficiary_Been_in_US_Yes_page6_37
      safeSelectRadio(
        form,
