...
/** low-level setters that tell us if the field existed */
function trySetText(form, field, value) {
  try { form.getTextField(field).setText(value ?? ''); return true; } catch { return false; }
}
function tryCheck(form, field, on = true) {
  try { const cb = form.getCheckBox(field); on ? cb.check() : cb.uncheck(); return true; } catch { return false; }
}

/** pick the first existing field name from a list */
function setTextOneOf(form, names, value) {
  for (const n of names) if (trySetText(form, n, value)) return true;
  return false;
}
function checkOneOf(form, names, on = true) {
  for (const n of names) if (tryCheck(form, n, on)) return true;
  return false;
}

function setText(form, name, value) { trySetText(form, name, value); }
function checkBox(form, name, on = true) { tryCheck(form, name, on); }
...
export function applyI129fMapping(saved, form) {
  ...
  /* ---------- Part 1 lines 1–5 (A#, acct, SSN, K-1/K-3 + I-130) ---------- */
  setTextOneOf(form, ['Pt1Line1_AlienNumber'], get(pet, 'aNumber'));
  setTextOneOf(form, ['Pt1Line2_AcctIdentifier'], get(pet, 'uscisOnlineAccount'));
  setTextOneOf(form, ['Pt1Line3_SSN'], get(pet, 'ssn'));

  const classif = (saved.classification || pet.classification || '').toLowerCase(); // 'k1' | 'k3'
  // K-1 checkbox candidates
  checkOneOf(form, [
    'Pt1Line4a_Checkboxes_p0_ch2',
    'Pt1Line4a_Checkbox_p0_ch2',
    'Pt1Line4a_K1',
    'Pt1Line4a'
  ], classif === 'k1');

  // K-3 checkbox candidates (sometimes 4b)
  checkOneOf(form, [
    'Pt1Line4b_Checkboxes_p0_ch2',
    'Pt1Line4b_Checkbox_p0_ch2',
    'Pt1Line4b_K3',
    'Pt1Line4b'
  ], classif === 'k3');

  // I-130 Yes/No when K-3 selected
  const filedI130 = !!(saved.k3FiledI130 ?? pet.k3FiledI130);
  checkOneOf(form, [
    'Pt1Line5_Checkboxes_p0_ch2', // Yes
    'Pt1Line5_Checkbox_p0_ch2',
    'Pt1Line5_Yes'
  ], classif === 'k3' && filedI130 === true);
  checkOneOf(form, [
    'Pt1Line5_Checkboxes_p0_ch3', // No
    'Pt1Line5_Checkbox_p0_ch3',
    'Pt1Line5_No'
  ], classif === 'k3' && filedI130 === false);

  /* ---------- Part 1 “Other Information” (21–26) ---------- */
  const sex = (pet.sex || '').toLowerCase(); // 'male' | 'female'
  const isMale = sex === 'm' || sex === 'male';
  const isFemale = sex === 'f' || sex === 'female';
  // Sex (21)
  checkOneOf(form, [
    'Pt1Line21_Checkbox_p2_ch2', 'Pt1Line21_Male', 'Pt1Line21_ch2'
  ], isMale);
  checkOneOf(form, [
    'Pt1Line21_Checkboxes_p2_ch3', 'Pt1Line21_Female', 'Pt1Line21_ch3'
  ], isFemale);

  // DOB (22)
  setTextOneOf(form, [
    'Pt1Line22_DateofBirth', 'Pt1Line22_DateOfBirth'
  ], fmtDate(pet.dob));

  // Marital status (23)
  const ms = (pet.maritalStatus || '').toLowerCase(); // single | married | divorced | widowed
  checkOneOf(form, ['Pt1Line23_Checkbox_p2_ch2','Pt1Line23_Single'],   ms==='single');
  checkOneOf(form, ['Pt1Line23_Checkbox_p2_ch3','Pt1Line23_Married'],  ms==='married');
  checkOneOf(form, ['Pt1Line23_Checkbox_p2_ch4','Pt1Line23_Divorced'], ms==='divorced');
  checkOneOf(form, ['Pt1Line23_Checkbox_p2_ch5','Pt1Line23_Widowed'],  ms==='widowed');

  // City/Province/Country of birth (24–26)
  setTextOneOf(form, [
    'Pt1Line24_CityTownOfBirth','Pt1Line24_CityTownVillageOfBirth','Pt1Line24_CityOfBirth'
  ], pet.birthCity || get(pet,'cityBirth'));
  setTextOneOf(form, [
    'Pt1Line25_ProvinceOrStateOfBirth','Pt1Line25_StateOfBirth','Pt1Line25_ProvinceOfBirth'
  ], pet.birthProvinceState || get(pet,'stateBirth') || get(pet,'provinceBirth'));
  setTextOneOf(form, [
    'Pt1Line26_CountryOfCitzOrNationality','Pt1Line26_CountryOfBirth'
  ], pet.birthCountry || get(pet,'countryBirth'));
  ...
  /* ---------- Parents (page 3) — add Sex + City + Country ---------- */
  ...
  checkOneOf(form, ['Pt1Line29_Checkbox_p2_ch2','Pt1Line29_Male','Pt1Line29_ch2'], p0Sex==='m'||p0Sex==='male');
  checkOneOf(form, ['Pt1Line29_Checkboxes_p2_ch3','Pt1Line29_Female','Pt1Line29_ch3'], p0Sex==='f'||p0Sex==='female');

  setTextOneOf(form, [
    'Pt1Line30_CityTownOfBirth','Pt1Line30_CityTownVillageOfBirth','Pt1Line30_CityOfBirth'
  ], p0.cityBirth || '');
  setTextOneOf(form, [
    'Pt1Line30_CountryOfCitzOrNationality','Pt1Line30_CountryOfBirth'
  ], p0.countryBirth || p0.nationality || '');
  ...
  checkOneOf(form, ['Pt1Line34_Checkbox_p2_ch2','Pt1Line34_Male','Pt1Line34_ch2'], p1Sex==='m'||p1Sex==='male');
  checkOneOf(form, ['Pt1Line34_Checkboxes_p2_ch3','Pt1Line34_Female','Pt1Line34_ch3'], p1Sex==='f'||p1Sex==='female');

  setTextOneOf(form, [
    'Pt1Line35_CityTownOfBirth','Pt1Line35_CityTownVillageOfBirth','Pt1Line35_CityOfBirth'
  ], p1.cityBirth || '');
  setTextOneOf(form, [
    'Pt1Line35_CountryOfCitzOrNationality','Pt1Line35_CountryOfBirth'
  ], p1.countryBirth || p1.nationality || '');
