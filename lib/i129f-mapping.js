...
/** Robust setters with fallbacks (silently skip if not present) */
...
export function applyI129fMapping(saved, form) {
  ...
  /* ===== Pt1 top items: A#, USCIS acct, SSN ===== */
  setFirstText(form, [
  ...
  if (isK1) {
    checkFirst(form, [
      'Pt1Line4a_Checkboxes_p0_ch0','Pt1Line4a_Checkboxes_p0_ch2','K1_Check','K1Visa','Part1_K1'
    ], true);
  } else if (isK3) {
    checkFirst(form, [
      'Pt1Line4a_Checkboxes_p0_ch1','Pt1Line4a_Checkboxes_p0_ch3','K3_Check','K3Visa','Part1_K3'
    ], true);
  }
  ...
  /* ===== Pt1 — Other Info (Sex / DOB / Marital / Birth Country) ===== */

  // SEX: checkbox pairs + text fallback
  ...
  if (sexVal === 'male' || sexVal === 'm') {
    ...
    setFirstText(form, ['Pt1Line21_Sex','Sex_Pt1','Part1_Sex_Text'], 'Male');
    ...
    setFirstText(form, ['Pt1Line21_Sex','Sex_Pt1','Part1_Sex_Text'], 'Female');
  }

  // DOB: try multiple state keys and multiple PDF field ids
  const petDobRaw = firstNonEmpty(pet.dob, pet.dateOfBirth, pet.birthDate, saved.petDob, saved.dateOfBirth);
  const petDob = fmtDate(petDobRaw);
  setFirstText(form, ['Pt1Line22_DateOfBirth','Pt1Line22_Date_of_Birth','DOB_Pt1','Pt1_DOB'], petDob);

  // Marital Status: checkbox sets + single text field fallback
  ...
  if (['single','never married','never-married','unmarried'].includes(ms)) {
    checkFirst(form, ['Pt1Line23_Single','Marital_Single_Pt1','Pt1_MS_Single'], true);
    setFirstText(form, ['Pt1Line23_MaritalStatus','MaritalStatus_Pt1','Pt1_MS_Text'], 'Single');
  } else if (ms === 'married') {
    checkFirst(form, ['Pt1Line23_Married','Marital_Married_Pt1','Pt1_MS_Married'], true);
    setFirstText(form, ['Pt1Line23_MaritalStatus','MaritalStatus_Pt1','Pt1_MS_Text'], 'Married');
  } else if (ms === 'divorced') {
    checkFirst(form, ['Pt1Line23_Divorced','Marital_Divorced_Pt1','Pt1_MS_Divorced'], true);
    setFirstText(form, ['Pt1Line23_MaritalStatus','MaritalStatus_Pt1','Pt1_MS_Text'], 'Divorced');
  } else if (ms === 'widowed') {
    checkFirst(form, ['Pt1Line23_Widowed','Marital_Widowed_Pt1','Pt1_MS_Widowed'], true);
    setFirstText(form, ['Pt1Line23_MaritalStatus','MaritalStatus_Pt1','Pt1_MS_Text'], 'Widowed');
  } else if (ms) {
    // Unknown string -> drop into text field
    setFirstText(form, ['Pt1Line23_MaritalStatus','MaritalStatus_Pt1','Pt1_MS_Text'], ms);
  }

  // City/Province were OK for you; add broader fallbacks for Country
  const petCityBirth = firstNonEmpty(pet.birthCity, pet.cityBirth, saved.birthCity);
  const petProvBirth = firstNonEmpty(pet.birthProvince, pet.stateBirth, pet.provinceBirth, saved.birthProvince, saved.stateBirth);
  const petCountryBirth = firstNonEmpty(
    pet.birthCountry, pet.countryBirth, pet.countryOfBirth, pet.nationality, saved.birthCountry, saved.countryOfBirth
  );

  setFirstText(form, ['Pt1Line24_CityTownOfBirth','Pt1Line24_City_Town_Of_Birth','Pt1_CityBirth'], petCityBirth);
  setFirstText(form, ['Pt1Line25_ProvinceOrStateOfBirth','Pt1_ProvinceBirth','Pt1_StateBirth'], petProvBirth);
  setFirstText(form, ['Pt1Line26_CountryOfBirth','Pt1_CountryBirth','Country_of_Birth_Pt1'], petCountryBirth);

  /* ===== PART 2 — BENEFICIARY (unchanged from your working set) ===== */
