...
// Where your template lives (your repo has public/i-129f.pdf)
...
// Try to set any field type by name
...
function trySelectRadio(form, fieldName, option) {
  if (!option) return false;
  const opt = String(option).replace(/^\//, '');
  try {
    const rg = form.getRadioGroup(fieldName);
    rg.select(opt);
    return true;
  } catch {
    // Some PDFs store options with a leading slash — try that too
    try {
      const rg = form.getRadioGroup(fieldName);
      rg.select(`/${opt}`);
      return true;
...
/**
 * Convert wizard shape -> logical keys expected by I129F_FIELD_MAP
 */
function buildLogicalFromWizard(data = {}) {
  ...
  const clsType = String(data.classification?.type ?? data.classification ?? 'k1').toLowerCase();
  ...
  const out = {
    ...
    // address history 1 (physicalAddresses[0])
    ...
    // radios
    class_type: clsType === 'k3' ? 'k3' : 'k1',
    k3_i130_filed: toBool(data.classification?.i130Filed),
...
export async function fillI129FPdf(inputData, options = {}) {
  ...
  if (!fields || fields.length === 0) {
    throw new Error('No AcroForm fields found in template.');
  }
  ...
  // Fill text
  for (const [logicalKey, pdfFieldName] of Object.entries(I129F_FIELD_MAP?.text || {})) {
    const ok = setAnyField(form, pdfFieldName, logical[logicalKey]);
    if (!ok) missing.push({ logicalKey, pdfFieldName, type: 'text' });
  }

  // Fill radios
  for (const [logicalKey, def] of Object.entries(I129F_FIELD_MAP?.radios || {})) {
    ...
    if (typeof v === 'boolean') {
      option = v ? (values.yes ?? values.true) : (values.no ?? values.false);
    } else {
      const key = String(v ?? '').toLowerCase();
      option = values[key] ?? values[v];
    }

    const ok = trySelectRadio(form, fieldName, option);
    if (!ok) missing.push({ logicalKey, pdfFieldName: fieldName, type: 'radio', option });
  }

  // Make filled text visible in viewers
  ...
  if (missing.length) {
    console.warn('[fillI129F] Missing/mismatched fields:', missing);
  }
