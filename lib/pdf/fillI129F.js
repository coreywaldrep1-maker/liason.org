...
// Try multiple common locations for the template
...
// Set any field type by name (text, checkbox, radio, dropdown)
...
/**
 * Convert your wizard state shape into the logical keys expected by I129F_FIELD_MAP.
 * Your wizard shape (from components/I129fWizard.jsx):
 * - petitioner.lastName, petitioner.firstName, petitioner.middleName, petitioner.aNumber, etc.
 * - mailing.street, mailing.city, ...
 * - physicalAddresses[], employment[]
 * - classification.type, classification.i130Filed, mailing.sameAsPhysical
 */
function buildLogicalFromWizard(data = {}) {
  ...
  out.__checks = {
    class_k1: clsType === 'k1',
    k3_i130_filed: clsType === 'k3' && toBool(data.classification?.i130Filed),
...
export async function fillI129FPdf(inputData, options = {}) {
  ...
  // Fill checkboxes
  for (const [logicalKey, pdfFieldName] of Object.entries(I129F_FIELD_MAP?.checks || {})) {
    const ok = setAnyField(form, pdfFieldName, !!logical.__checks?.[logicalKey]);
    if (!ok) missing.push({ logicalKey, pdfFieldName });
  }

  // ✅ THIS IS THE BIG ONE: make text visible in most viewers
  const helv = await pdfDoc.embedFont(StandardFonts.Helvetica);
  try {
    form.updateFieldAppearances(helv);
  } catch {}

  // Flatten by default (ensures filled values render consistently)
